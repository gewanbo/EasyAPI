...
if [ -f ${EASY_HOME}/bin/easyclient.pid ] ; then
	echo "found easyclient.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
### Set app version
libFile=`ls -l ${EASY_HOME}/lib/easyapi.client*|awk '{print $9}'`
libFile1=${libFile#*-}
appVersion=${libFile1%-*}

JAVA_OPTS=" $JAVA_OPTS -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dfile.encoding=UTF-8"
EASY_OPTS="-DappName=easyclient -DappVersion=$appVersion -Dlogback.configurationFile=$logback_configurationFile -Deasy.conf=$easy_conf"
...
then 
	...
	${RUNNER} ${JAVA_OPTS} ${EASY_OPTS} -classpath .:${LAUNCH_CLASSPATH} com.wanbo.easyapi.client.EasyClient 1>>${EASY_HOME}/logs/out.log 2>&1 &
